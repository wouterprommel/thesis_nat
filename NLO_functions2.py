...
def q_2_s(u, x, Q2, flavour):
    return pdf.xfxQ2(flavour, x/u, Q2)
...
def Ca(x, Q2, flavour):
    ...
    f2 = lambda z: np.log(1 - z)/(1-z)
    r2, err = integrate.quad(f2, 0, x, epsabs=PRECISION)
    return r - 2 * q_s(x, Q2, flavour) * r2
...
def Cd(x, Q2, flavour, i):
    if i == 1:
        f = lambda z: (3/z) * q_s(x/z, Q2, flavour) 
    else:
        f = lambda z: (3/z + 2) * q_s(x/z, Q2, flavour) 
    r, err = integrate.quad(f, x, 1, epsabs=PRECISION)
    #r = MC(f, x, 1)
    return r
